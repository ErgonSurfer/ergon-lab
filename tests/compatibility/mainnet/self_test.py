...
class SmokeContractTest(unittest.TestCase):
    ...
    def test_bounded_wait_checks_network_surface_and_enforces_disk_cap(self):
        spec = SMOKE.NodeSpec(
        ...
        with mock.patch.object(SMOKE, "directory_bytes", return_value=0), \
            ...
            SMOKE.wait_for_bounded_exit(node, self.root)

        process = mock.Mock()
        process.poll.return_value = 0
        node = SMOKE.NodeProcess(spec, process)
        with mock.patch.object(SMOKE, "rpc") as rpc_call:
            SMOKE.wait_for_bounded_exit(node, self.root)
        ...
        node = SMOKE.NodeProcess(spec, process)
        with mock.patch.object(SMOKE, "directory_bytes", return_value=0), \
                mock.patch.object(SMOKE, "rpc", return_value={
                    "connections": 1,
                    "networkactive": True,
                    "localrelay": True,
                    "localaddresses": [],
    ...
    def test_snapshot_fail_closed_oracles(self):
        ...
        def side_effect(_spec, method, params=None):
            values = {
                ...
                "getblockhash": SMOKE.EXPECTED_GENESIS if params == [0] else "1" * 64,
